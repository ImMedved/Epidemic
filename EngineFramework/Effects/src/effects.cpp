    ...
    core::tasks::ITaskScheduler* scheduler)
{
    ...
    {
        const bool all = std::all_of(prepared.begin(), prepared.end(), [](const PreparedOperation& item) { return item.commit; });
        if (!all)
        {
    ...
    core::tasks::ITaskScheduler* scheduler)
{
    ...
    RecordChange(EffectChange{0, EffectChangeKind::ExecutionCompleted, execution, {}, {}, {},
                              any_failed ? EffectOperationDisposition::Failed : EffectOperationDisposition::Applied,
                              request.context});
...
{
    if (FindDefinition(request.definition) == nullptr || !clock.IsValid() || request.targets.empty())
    {
        return foundation::Result<DeferredEffectId>::Failure(Error("gameplay.deferred_effect_invalid", "deferred effect requires registered definition, clock and targets"));
    }
    ...
    RecordChange(EffectChange{0, EffectChangeKind::DeferredCreated, {}, {}, {}, id, EffectOperationDisposition::Applied, {}});
