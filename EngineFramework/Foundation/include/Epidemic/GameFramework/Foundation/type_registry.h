...
template <typename TId> class StableTypeRegistry
{
  public:
    ...
    [[nodiscard]] foundation::Result<TId> Register(std::string_view canonical_name)
    {
        ...
        {
            return foundation::Result<TId>::Failure(
                foundation::Error::Create("gameplay.invalid_name", "canonical name must not be empty"));
        }
        ...
        if (found != entries_.end())
        {
            ...
            return foundation::Result<TId>::Failure(
                foundation::Error::Create("gameplay.already_registered", "canonical name is already registered", std::string(canonical_name)));
        }
