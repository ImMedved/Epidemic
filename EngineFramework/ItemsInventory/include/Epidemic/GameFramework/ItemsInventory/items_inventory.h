...
struct RegisteredPayload
{
    ...
    template<class T> [[nodiscard]] static RegisteredPayload FromTrivial(TypeId type_id,const T& value)
    {
        ...
        RegisteredPayload out; out.type=type_id; out.bytes.resize(sizeof(T)); std::memcpy(out.bytes.data(),&value,sizeof(T)); return out;
    }
...
enum class ItemStackPolicy { NonStackable, StackByDefinition, StackByEquivalentState, CustomRegistered };
enum class ItemDurabilityPolicy { None, InstanceValue };
enum class ItemChargePolicy { None, InstanceValue };
enum class ItemLocationKind { None, Container, World, Equipped, Reserved, InTransit, Destroyed };
enum class ContainerState { Active, Locked, Disabled, Destroyed };
enum class ReservationState { Active, Consumed, Released };
enum class ItemChangeKind { ItemCreated, ItemDestroyed, StackSplit, StacksMerged, TransferCommitted, ContainerCreated, ReservationCreated, ReservationReleased, ReservationConsumed, DurabilityChanged, ChargesChanged };

struct ItemProperty { ItemPropertyTypeId type{}; RegisteredPayload payload; [[nodiscard]] bool operator==(const ItemProperty&) const = default; };
...
struct ContainerUsage { Fixed weight=0; Fixed volume=0; std::uint32_t slots=0; };
...
struct ItemChange { std::uint64_t sequence=0; ItemChangeKind kind=ItemChangeKind::ItemCreated; ItemInstanceId item{}; ContainerId container{}; Fixed quantity=0; GameplayContext context{}; Revision revision{}; };
...
struct ItemsDiagnostics { std::uint64_t items=0; std::uint64_t containers=0; std::uint64_t active_reservations=0; std::uint64_t transfers=0; std::uint64_t rejected_transfers=0; };

class ItemsInventoryService
{
public:
    ItemsInventoryService();
    [[nodiscard]] static constexpr GameplayDomainId Domain() noexcept { return GameplayDomainId::FromString("framework.items_inventory"); }
    ...
    [[nodiscard]] foundation::Result<ItemInstanceId> SplitStack(ItemInstanceId item,Fixed quantity,GameplayContext context={});
    [[nodiscard]] foundation::Result<void> MergeStacks(ItemInstanceId target,ItemInstanceId source,GameplayContext context={});

    [[nodiscard]] foundation::Result<ItemTransferPlan> PrepareTransfer(ItemInstanceId item,ItemLocation target,Fixed quantity,GameplayContext context={});
    ...
    [[nodiscard]] foundation::Result<ItemReservationId> ReserveItem(ItemInstanceId item,Fixed quantity,GameplayObjectRef owner,TypeId reason={},GameplayContext context={});
    [[nodiscard]] foundation::Result<void> ReleaseReservation(ItemReservationId reservation,GameplayContext context={});
    [[nodiscard]] foundation::Result<void> ConsumeReservation(ItemReservationId reservation,GameplayContext context={});
    ...
    [[nodiscard]] foundation::Result<void> AdjustDurability(ItemInstanceId item,Fixed delta,GameplayContext context={});
    [[nodiscard]] foundation::Result<void> AdjustCharges(ItemInstanceId item,Fixed delta,GameplayContext context={});
...
private:
    ...
    [[nodiscard]] bool ValidateContainerTarget(ItemInstanceId moving,ContainerId target,Fixed quantity) const noexcept;
