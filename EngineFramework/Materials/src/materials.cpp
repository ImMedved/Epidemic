...
foundation::Result<void> MaterialService::ApplySubstanceExposure(
    ...
    GameplayContext context)
{
    if (FindSubstance(substance) == nullptr || amount_micro <= 0 || coverage_ppm > kCompositionOnePpm)
    {
    ...
    {
        exposure = exposures.insert(exposure, SubstanceExposure{substance, amount_micro, coverage_ppm});
    }
    else
    {
        exposure->amount_micro = SaturatingAdd(exposure->amount_micro, amount_micro);
...
std::int64_t MaterialService::ThresholdValue(
    ...
    ReactionThresholdField field) const noexcept
{
    switch (field)
    {
    ...
    case ReactionThresholdField::ExposureAmount:
    {
        ...
        for (const auto& exposure : state.dynamic.exposures)
        {
            amount = SaturatingAdd(amount, exposure.amount_micro);
        }
        return amount;
    }
    }
    ...
    const GameplayTagRegistry& tags) const
{
    ...
    for (const auto reaction_id : reaction_order_)
    {
        ...
        if (ThresholdValue(state, stimulus, rule.threshold_field) < rule.min_threshold_micro)
        {
            continue;
        }
    ...
    const GameplayTagRegistry& tags)
{
    ...
    case MaterialStimulusType::Moisture:
        state.dynamic.saturation_ppm = ClampPpm(static_cast<std::int64_t>(state.dynamic.saturation_ppm) + stimulus.magnitude_micro);
        break;
    case MaterialStimulusType::Drying:
        state.dynamic.saturation_ppm = ClampPpm(static_cast<std::int64_t>(state.dynamic.saturation_ppm) - stimulus.magnitude_micro);
        break;
    case MaterialStimulusType::Corrosive:
        state.dynamic.corrosion_ppm = ClampPpm(static_cast<std::int64_t>(state.dynamic.corrosion_ppm) + stimulus.magnitude_micro);
        break;
...
const MaterialSlotState* MaterialService::FindState(GameplayObjectRef subject, MaterialSlotId slot) const noexcept
{
...
foundation::Result<void> MaterialService::RestoreSnapshot(MaterialsSnapshot snapshot)
{
    ...
    for (auto& state : snapshot.states)
    {
        ...
        for (const auto& exposure : state.dynamic.exposures)
        {
            if (FindSubstance(exposure.substance) == nullptr || exposure.coverage_ppm > kCompositionOnePpm)
            {
