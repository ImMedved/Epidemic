...
OwnershipCrimeAdapter::OwnershipCrimeAdapter(ownership::OwnershipService &ownership,
                                             crime::CrimeService &crime) noexcept
    : ownership_(ownership), crime_(crime)
{
}
foundation::Result<TheftResolution> OwnershipCrimeAdapter::TryCreateCrimeFromDeniedRight(
    GameplayObjectRef actor, GameplayObjectRef property, ownership::PropertyRightId right,
    crime::CrimeTypeId crime_type, GameplayObjectRef area, GameplayContext context)
{
    auto permission = ownership_.EvaluatePermission({actor, property, right, context});
    TheftResolution out;
    out.permission = permission;
    if (permission.decision == ownership::PermissionDecision::Allowed ||
        permission.decision == ownership::PermissionDecision::NotApplicable)
    {
        return foundation::Result<TheftResolution>::Success(out);
    }
    crime::CrimeCandidate candidate;
    candidate.type = crime_type;
    candidate.offender = actor;
    candidate.victim = permission.effective_owner;
    candidate.target_property = property;
    candidate.area = area;
    candidate.time =
        context.tick.IsValid() ? GameplayTimePoint{static_cast<std::int64_t>(context.tick.Raw())} : GameplayTimePoint{};
...
CrimeSocietyAdapter::CrimeSocietyAdapter(crime::CrimeService &crime, society::SocietyService &society) noexcept
                                                                       ...
                                                                       GameplayContext context)
{
    ...
    return society_.ApplySocialChange({record->victim, record->offender, relationship_type, delta_micro,
                                       TypeId::FromString("framework.crime.relationship_penalty"), context});
}
foundation::Result<crime::LawResponseId> CrimeSocietyAdapter::GenerateAuthorityIntentLikeResponse(
    crime::CrimeRecordId crime_id, crime::LawResponseTypeId response_type, GameplayObjectRef authority,
