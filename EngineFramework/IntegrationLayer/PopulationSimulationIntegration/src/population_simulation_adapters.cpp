...
PopulationBackedSpawnResult PopulationEncounterAdapter::SpawnFromPopulation(population::PopulationService &pop,
                                                                            ...
                                                                            std::size_t max_units) const
{
    ...
    {
        if (result.allocated_units.size() >= max_units)
            break;
        if (unit.state == population::PopulationUnitState::Latent ||
            unit.state == population::PopulationUnitState::Abstract)
        {
    ...
    {
        auto materialized =
            pop.MaterializeUnit(result.allocated_units[index], result.spawn.created_entities[index], request.context);
        if (materialized)
        {
            result.spawn.linked_population_units.push_back(
                GameplayObjectRef{population::PopulationService::Domain(), result.allocated_units[index].value});
        }
    }
    return result;
}
                                                                      ...
                                                                      GameplayContext context) const
{
    if (!duty.subject.IsValid())
        return foundation::Result<void>::Failure(Error("gameplay.population_sim.invalid_duty", "invalid duty"));
...
foundation::Result<void> RolesNeedsAdapter::SatisfyWorkNeedFromDuty(const roles_jobs::Duty &duty,
                                                                    ...
                                                                    std::int64_t amount, GameplayContext context) const
{
    if (!duty.subject.IsValid())
        return foundation::Result<void>::Failure(Error("gameplay.population_sim.invalid_duty", "invalid duty"));
                                                                      ...
                                                                      GameplayContext context) const
{
    ...
    const auto subject =
        record->entity.value_or(GameplayObjectRef{population::PopulationService::Domain(), unit.value});
...
CityMorningResult CityLifeAdapter::RunMorningStep(population::PopulationService &pop,
                                                  roles_jobs::RolesJobsService &roles,
                                                  needs_life::NeedsLifeService &needs, GameplayObjectRef area,
                                                  GameplayTimePoint now, std::size_t materialization_limit,
                                                  GameplayContext context) const
{
