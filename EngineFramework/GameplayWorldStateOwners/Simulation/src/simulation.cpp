...
foundation::Result<SimulationRegionId> SimulationService::RegisterRegion(SimulationRegion region)
{
    if (frozen_)
        return foundation::Result<SimulationRegionId>::Failure(
            Error("gameplay.simulation.registry_frozen", "simulation registry frozen"));
...
bool SimulationService::DetailAllows(const SimulationLayerDefinition &layer,
                                                                            ...
                                                                            GameplayContext context)
{
    ...
    if (to.ticks < from.ticks)
        return foundation::Result<SimulationSummaryId>::Failure(
            Error("gameplay.simulation.invalid_interval", "invalid simulation interval"));
    if (budget_.max_interval_ticks > 0 && to.ticks - from.ticks > budget_.max_interval_ticks)
        return foundation::Result<SimulationSummaryId>::Failure(
    ...
    if (ordered.size() > budget_.max_tasks)
    {
        ...
        return foundation::Result<SimulationSummaryId>::Failure(
            Error("gameplay.simulation.task_budget", "simulation task budget exceeded"));
    }
    SimulationSummary summary;
    summary.id = SimulationSummaryId{summary_ids_.Next()};
    summary.region = region_id;
    summary.from = from;
    summary.to = to;
    ...
    {
        SimulationTask task;
        task.id = SimulationTaskId{task_ids_.Next()};
        task.region = region_id;
        task.layer = layer.id;
        task.from = from;
        task.to = to;
        task.detail = region_it->second.detail_level;
        task.state = SimulationTaskState::Pending;
        Bump();
        ...
        if (commits >= budget_.max_layer_commits)
        {
            ...
            Record({0, SimulationChangeKind::TaskDeferred, region_id, layer.id, task.id, context.time, context,
                    revision_});
            continue;
        }
        ...
        if (!prepared)
        {
            ...
            Record(
                {0, SimulationChangeKind::TaskFailed, region_id, layer.id, task.id, context.time, context, revision_});
            continue;
        }
        ...
        Record(
            {0, SimulationChangeKind::TaskCommitted, region_id, layer.id, task.id, context.time, context, revision_});
    }
    Bump();
    summary.revision = revision_;
    region_it->second.last_simulated_at = to;
    region_it->second.revision = revision_;
...
{
    std::vector<SimulationChange> out;
...
{
    regions_.clear();
    pending_tasks_ = std::move(s.pending_tasks);
    summaries_ = std::move(s.summaries);
