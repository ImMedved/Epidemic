...
foundation::Result<ItemInstanceId> ItemsInventoryService::CreateItem(ItemInstance item, GameplayContext context)
{
    ...
    if (!item.id.IsValid())
        item.id = ItemInstanceId{item_ids_.Next()};
    ...
    ++diagnostics_.items;
    Record({0, ItemChangeKind::ItemCreated, id, {}, 1, context, revision_});
...
foundation::Result<void> ItemsInventoryService::DestroyItem(ItemInstanceId id, GameplayContext context)
{
    ...
    if (diagnostics_.items > 0)
        --diagnostics_.items;
    Record({0, ItemChangeKind::ItemDestroyed, id, {}, 0, context, revision_});
                                                            ...
                                                            GameplayContext context)
{
    ...
    Bump();
    t->second.quantity += s->second.quantity;
...
ContainerUsage ItemsInventoryService::GetContainerUsage(ContainerId id) const noexcept
{
    ...
    for (const auto &[iid, item] : items_)
    {
        ...
        if (!def)
            continue;
        out.weight += def->base_weight * item.quantity;
        out.volume += def->base_volume * item.quantity;
        ++out.slots;
    }
    return out;
}
                                                    ...
                                                    Fixed quantity) const noexcept
{
    ...
    if (item)
    {
        ...
        if (!def)
            return false;
        usage.weight += def->base_weight * quantity;
        usage.volume += def->base_volume * quantity;
        ++usage.slots;
    }
...
foundation::Result<void> ItemsInventoryService::ReleaseReservation(ItemReservationId id, GameplayContext context)
{
    ...
    if (diagnostics_.active_reservations > 0)
        --diagnostics_.active_reservations;
    Record({0, ItemChangeKind::ReservationReleased, it->second.item, {}, it->second.quantity, context, revision_});
    return foundation::Result<void>::Success();
}
foundation::Result<void> ItemsInventoryService::ConsumeReservation(ItemReservationId id, GameplayContext context)
{
    ...
    it->second.state = ReservationState::Consumed;
    it->second.revision = revision_;
    ...
    if (diagnostics_.active_reservations > 0)
        --diagnostics_.active_reservations;
    Record({0, ItemChangeKind::ReservationConsumed, iid, {}, qty, context, revision_});
    return foundation::Result<void>::Success();
}
                                                                 ...
                                                                 GameplayContext context)
{
    ...
    Bump();
    it->second.durability = std::clamp(it->second.durability + delta, Fixed{0}, d->max_durability);
    it->second.revision = revision_;
    Record({0, ItemChangeKind::DurabilityChanged, id, {}, delta, context, revision_});
    return foundation::Result<void>::Success();
}
foundation::Result<void> ItemsInventoryService::AdjustCharges(ItemInstanceId id, Fixed delta, GameplayContext context)
{
    ...
    Bump();
    it->second.charges = std::clamp(it->second.charges + delta, Fixed{0}, d->max_charges);
    it->second.revision = revision_;
    Record({0, ItemChangeKind::ChargesChanged, id, {}, delta, context, revision_});
    return foundation::Result<void>::Success();
}
