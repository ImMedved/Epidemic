...
foundation::Result<OwnershipRecordId> OwnershipService::AssignOwnership(OwnershipRecord r)
{
    ...
    Record({0, OwnershipChangeKind::OwnershipAssigned, r.property, {}, r.owner, {}, {}, revision_});
...
foundation::Result<void> OwnershipService::RemoveOwnership(OwnershipRecordId id, GameplayContext c)
{
    ...
    auto property = it->second.property;
    auto owner = it->second.owner;
    records_.erase(it);
    Bump();
    Record({0, OwnershipChangeKind::OwnershipRemoved, property, {}, owner, {}, c, revision_});
...
foundation::Result<void> OwnershipService::TransferOwnership(TransferOwnershipRequest q)
{
    ...
    auto *current = const_cast<OwnershipRecord *>(GetOwner(q.property));
    if (!current)
    {
        OwnershipRecord r;
        r.property = q.property;
        r.owner = q.to_owner;
        r.strength = q.reason == TransferReason::Theft ? OwnershipStrength::Stolen : OwnershipStrength::Owned;
        auto added = AssignOwnership(r);
        if (!added)
            return foundation::Result<void>::Failure(std::move(added.GetError()));
        return foundation::Result<void>::Success();
    }
    if (q.from_owner.IsValid() && current->owner != q.from_owner)
        return foundation::Result<void>::Failure(
            Error("gameplay.ownership.transfer_owner_mismatch", "transfer owner mismatch"));
    Bump();
    current->owner = q.to_owner;
    current->strength = q.reason == TransferReason::Theft ? OwnershipStrength::Stolen : OwnershipStrength::Owned;
    current->revision = revision_;
    ++diagnostics_.ownership_transfers;
    Record({0, OwnershipChangeKind::OwnershipTransferred, q.property, {}, q.to_owner, {}, q.context, revision_});
    return foundation::Result<void>::Success();
}
...
foundation::Result<AccessRuleId> OwnershipService::AddAccessRule(AccessRule r)
{
    ...
    auto id = r.id;
    rules_.emplace(id, r);
    Record({0, OwnershipChangeKind::AccessRuleAdded, r.property, r.required_subject, {}, r.right, {}, revision_});
...
OwnershipPermissionResult OwnershipService::EvaluatePermission(OwnershipPermissionQuery q) const
{
    ...
    auto owner = GetOwner(q.property);
    if (owner)
    ...
    {
        (void)id;
        if (g.subject == q.actor && g.property == q.property && g.right == q.right)
        {
            out.decision = PermissionDecision::Allowed;
            return out;
        }
...
{
    const OwnershipRecord *best = nullptr;
    for (const auto &[id, r] : records_)
    {
        (void)id;
        if (r.property == property &&
            (r.strength == OwnershipStrength::Owned || r.strength == OwnershipStrength::Public ||
             r.strength == OwnershipStrength::Claimed || r.strength == OwnershipStrength::Stolen))
        {
            if (!best || r.id < best->id)
                best = &r;
        }
    }
    return best;
...
{
    std::vector<OwnershipChange> out;
...
OwnershipSnapshot OwnershipService::CaptureSnapshot() const
{
    ...
    std::sort(s.records.begin(), s.records.end(), [](auto &a, auto &b) { return a.id < b.id; });
    std::sort(s.grants.begin(), s.grants.end(), [](auto &a, auto &b) { return a.id < b.id; });
    std::sort(s.rules.begin(), s.rules.end(), [](auto &a, auto &b) { return a.id < b.id; });
    std::sort(s.claims.begin(), s.claims.end(), [](auto &a, auto &b) { return a.id < b.id; });
...
foundation::Result<void> OwnershipService::RestoreSnapshot(OwnershipSnapshot s)
{
    ...
    {
        if (!r.id.IsValid() || !r.property.IsValid() || !r.owner.IsValid())
            return foundation::Result<void>::Failure(
                Error("gameplay.ownership.restore_invalid", "invalid ownership snapshot"));
        records_[r.id] = r;
    }
    for (auto &g : s.grants)
    {
        if (!g.id.IsValid())
            return foundation::Result<void>::Failure(
                Error("gameplay.ownership.restore_invalid", "invalid grant snapshot"));
        grants_[g.id] = g;
    }
    for (auto &r : s.rules)
    {
        if (!r.id.IsValid())
            return foundation::Result<void>::Failure(
                Error("gameplay.ownership.restore_invalid", "invalid rule snapshot"));
        rules_[r.id] = r;
    }
    for (auto &c : s.claims)
    {
        if (!c.id.IsValid())
            return foundation::Result<void>::Failure(
                Error("gameplay.ownership.restore_invalid", "invalid claim snapshot"));
        claims_[c.id] = c;
    }
