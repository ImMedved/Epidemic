...
foundation::Result<CreateEntityResult> EntityService::Create(CreateEntityRequest request)
{
    if (!frozen_)
    {
        return foundation::Result<CreateEntityResult>::Failure(Error("gameplay.registry_not_frozen", "entity service must be frozen before runtime operations"));
    }
...
foundation::Result<void> EntityService::Deactivate(EntityId id, GameplayContext context)
{
    ...
    {
        return foundation::Result<void>::Failure(Error("gameplay.entity_invalid_state", "only alive entities can become dormant"));
    }
    record->lifecycle = EntityLifecycleState::Dormant;
    record->materialization = EntityMaterializationState::Abstract;
    BumpRevision(*record);
    RecordChange(EntityChange{0, EntityChangeKind::MaterializationChanged, id, {}, record->archetype, record->lifecycle,
                              EntityMaterializationState::Abstract, record->materialization, {}, EntityDestroyReason::OwnerRequest, context});
...
foundation::Result<void> EntityService::Activate(EntityId id, GameplayContext context)
{
    ...
    if (record->lifecycle != EntityLifecycleState::Dormant)
    {
        return foundation::Result<void>::Failure(Error("gameplay.entity_invalid_state", "only dormant entities can be activated"));
    }
    record->lifecycle = EntityLifecycleState::Alive;
    BumpRevision(*record);
    RecordChange(EntityChange{0, EntityChangeKind::MaterializationChanged, id, {}, record->archetype, record->lifecycle,
                              record->materialization, record->materialization, {}, EntityDestroyReason::OwnerRequest, context});
    return foundation::Result<void>::Success();
}
...
std::vector<EntityRecord> EntityService::CommitPendingDestruction()
{
    ...
    for (const auto id : pending_destroy_)
    {
        ...
        slot.record = final_record;
        slot.record.materialization = EntityMaterializationState::Abstract;
    ...
    GameplayContext context)
{
    ...
    if (record->archetype == new_archetype && policy.preserve_instance_tags && policy.preserve_persistence)
    {
        return foundation::Result<void>::Success();
    }
    ...
    GameplayContext context)
{
    ...
    if (record->lifecycle != EntityLifecycleState::Alive)
    {
        return foundation::Result<void>::Failure(Error("gameplay.entity_invalid_state", "only alive entities can change materialization state"));
    }
...
EntitySnapshot EntityService::CaptureSnapshot() const
{
    ...
    for (const auto& slot : slots_)
    {
        if (slot.occupied && slot.record.persistence != EntityPersistencePolicy::Transient)
        {
...
foundation::Result<void> EntityService::RestoreSnapshot(EntitySnapshot snapshot)
{
    ...
    {
        if (!record.id.IsValid() || FindArchetype(record.archetype) == nullptr || seen.contains(record.id) || record.revision > snapshot.revision)
        {
