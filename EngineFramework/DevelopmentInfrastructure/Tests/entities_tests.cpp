...
int main()
{
    ...
    if (destroyed.size() != 1 || service.Find(first_id) == nullptr || service.Find(first_id)->lifecycle != EntityLifecycleState::Destroyed ||
        service.Resolve(first_handle) == nullptr) return 13;
    ...
    if (restored == nullptr || restored->archetype != object_id.Value() || restored->materialization != EntityMaterializationState::Abstract) return 16;
    if (service.HandleOf(first_id).generation == 0 || service.Resolve(first_handle) != nullptr) return 17;
    ...
    if (!service.Deactivate(first_id) || service.Find(first_id)->lifecycle != EntityLifecycleState::Dormant) return 171;
    if (service.SetMaterializationState(first_id, EntityMaterializationState::Materialized)) return 172;
    if (!service.Activate(first_id) || service.Find(first_id)->lifecycle != EntityLifecycleState::Alive) return 173;
