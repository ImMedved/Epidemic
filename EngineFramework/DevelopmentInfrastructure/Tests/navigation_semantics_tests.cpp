...
int main()
{
    ...
    const auto restore_report = epidemic::tests::restore_fault::RunObservedRestoreSweep(
        "NavigationSemantics.RestoreSnapshot",
        2,
        ...
        [&](const epidemic::tests::allocation_fault::SweepIteration &iteration, const auto &allocation_baseline) {
            ...
            const auto pre_state =
                epidemic::tests::pre_state::StateComparator("NavigationSemantics.RestoreSnapshot.pre_state")
                    .RequireEqual(epidemic::tests::pre_state::StateFacet::PrimaryRecords,
                                  allocation_baseline.profiles.size() + allocation_baseline.layers.size() +
                                      allocation_baseline.links.size(),
                                  allocation_after.profiles.size() + allocation_after.layers.size() +
                                      allocation_after.links.size(),
                                  "profile layer link record count")
                    .RequireEqual(epidemic::tests::pre_state::StateFacet::RecordPayloads,
                                  allocation_baseline.links.size(), allocation_after.links.size(), "link payload count")
                    .RequireEqual(epidemic::tests::pre_state::StateFacet::SecondaryIndexes,
                                  diagnostics.links, static_cast<std::uint64_t>(allocation_baseline.links.size()),
                                  "link index diagnostics")
                    .RequireEqual(epidemic::tests::pre_state::StateFacet::IdGenerators,
                                  allocation_baseline.layer_ids.next, allocation_after.layer_ids.next,
                                  "layer id generator next")
                    .RequireEqual(epidemic::tests::pre_state::StateFacet::Revisions,
                                  allocation_baseline.revision, allocation_after.revision, "revision")
                    .RequireEqual(epidemic::tests::pre_state::StateFacet::Journal,
                                  allocation_baseline.journal.size(), allocation_after.journal.size(),
                                  "journal entry count")
                    .RequireEqual(epidemic::tests::pre_state::StateFacet::JournalEpoch,
                                  allocation_baseline.change_epoch, allocation_after.change_epoch, "journal epoch")
                    .RequireEqual(epidemic::tests::pre_state::StateFacet::JournalSequence,
                                  allocation_baseline.next_change_sequence, allocation_after.next_change_sequence,
                    ...
                    .RequireEqual(epidemic::tests::pre_state::StateFacet::JournalLatestCursor,
                                  restored.LatestChangeCursor(), restored.LatestChangeCursor(), "latest cursor")
                    .RequireEqual(epidemic::tests::pre_state::StateFacet::PublicReadModels,
                                  diagnostics.layers, static_cast<std::uint64_t>(allocation_baseline.layers.size()),
