...
class Part final : public ISaveParticipant
{
  public:
    ...
    epidemic::foundation::Result<SaveSection> CaptureSnapshot(const SaveContext &) const override
    {
        ...
        s.payload.resize(sizeof(value));
        std::memcpy(s.payload.data(), &value, sizeof(value));
        return epidemic::foundation::Result<SaveSection>::Success(std::move(s));
    }
    epidemic::foundation::Result<void> ValidateSnapshot(const SaveSection &s, const RestoreContext &) const override
    {
        return s.payload.size() == sizeof(int) ? epidemic::foundation::Result<void>::Success()
                                               : epidemic::foundation::Result<void>::Failure(
                                                     epidemic::foundation::Error::Create("invalid", "invalid"));
    }
    epidemic::foundation::Result<std::unique_ptr<IRestoreStage>> StageRestore(const SaveSection &s,
                                                                              const RestoreContext &) override
    {
        ...
        std::memcpy(&st->value, s.payload.data(), sizeof(int));
        return epidemic::foundation::Result<std::unique_ptr<IRestoreStage>>::Success(std::move(st));
    }
    epidemic::foundation::Result<void> CommitRestore(IRestoreStage &s) override
    {
        value = static_cast<Stage &>(s).value;
...
int main()
{
    ...
    b.value = 22;
    b.deps = {a.id};
    SaveGameOrchestrator o;
    Check(static_cast<bool>(o.RegisterParticipant(a)), "register a");
    Check(static_cast<bool>(o.RegisterParticipant(b)), "register b");
    auto order = o.ResolveOrder();
    Check(static_cast<bool>(order) && order.Value().size() == 2 && order.Value()[0] == a.id, "dependency order");
    SaveContext sc;
    sc.content_version = 4;
    sc.content_hash = 99;
    auto image = o.Capture(sc);
    Check(static_cast<bool>(image), "capture");
    a.value = 100;
    b.value = 200;
    b.fail_stage = true;
    RestoreContext rc;
    rc.current_content_version = 4;
    rc.current_content_hash = 99;
    Check(!o.Restore(image.Value(), rc), "staging failure");
    Check(a.value == 100 && b.value == 200, "staging failure leaves live state untouched");
    b.fail_stage = false;
    Check(static_cast<bool>(o.Restore(image.Value(), rc)), "restore");
