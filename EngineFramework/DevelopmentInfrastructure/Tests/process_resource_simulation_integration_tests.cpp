...
int main()
{
    ...
    auto stockpile = resources.CreateStockpile({{}, owner, {}, {}, StockpileState::Active, {}});
    if (!stockpile)
        return 2;
    if (!resources.Add(stockpile.Value(), {iron_id.Value(), 10}))
        return 11;
    ...
    def.steps.push_back(
        {ProcessStepId::FromString("test.step"), TypeId::FromString("test.step"), GameplayDuration{5}, {}, {}});
    ...
    recipe.inputs.push_back(
        {ProcessInputId::FromString("test.input.iron"),
         ProcessInputTypeId::FromString("framework.input.resource"),
         2,
         InputConsumptionPolicy::ReserveThenConsume,
         {},
         RegisteredPayload::FromTrivial(ResourceProcessInputProvider::PayloadType(), iron_payload)});
    recipe.outputs.push_back(
        {ProcessOutputId::FromString("test.output.sword"), ResourceProcessOutputHandler::OutputType(), 1,
         OutputDeliveryPolicy::OnCompletion,
         RegisteredPayload::FromTrivial(ResourceProcessInputProvider::PayloadType(), sword_payload)});
    auto recipe_id = processes.RegisterRecipe(recipe);
    if (!recipe_id)
        return 4;
    auto proc = processes.StartProcess({recipe_id.Value(), owner, {}, {}, GameplayTimePoint{0}, 1, {}});
    if (!proc)
        return 5;

    SimulationService sim;
    ProcessesSimulationLayer process_layer(processes);
    auto region = sim.RegisterRegion({SimulationRegionId::FromString("test.region"),
                                      "test.region",
                                      owner,
    ...
    if (!sim.RegisterLayer({ProcessesSimulationLayer::StaticLayer(),
                           ...
                           &process_layer))
        return 7;
    auto summary = sim.SimulateInterval(region.Value(), GameplayTimePoint{0}, GameplayTimePoint{5});
    if (!summary)
        return 8;
    if (resources.GetAmount(stockpile.Value(), iron_id.Value()) != 8 ||
        resources.GetAmount(stockpile.Value(), sword_id.Value()) != 1)
        return 9;
    for (int i = 0; i < 100; ++i)
    {
