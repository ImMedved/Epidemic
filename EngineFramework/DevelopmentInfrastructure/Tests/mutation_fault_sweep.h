...
namespace epidemic::tests::mutation_fault
{
template <typename TMakeFixture, typename TInvoke, typename TCapture, typename TVerify>
[[nodiscard]] allocation_fault::SweepReport RunObservedMutationSweep(std::string api,
                                                                     long long observed_allocation_spare,
                                                                     TMakeFixture &&make_fixture,
                                                                     TInvoke &&invoke,
                                                                     TCapture &&capture,
                                                                     TVerify &&verify)
{
    const auto probe = allocation_fault::CountObservedAllocations([&] {
        ...
        return [&, fixture = std::move(fixture)](allocation_fault::SweepRunContext &context) mutable {
            context.MarkMethodInvoked();
            return invoke(fixture, context);
        };
    });
    ...
    for (long long fault_index = 0; fault_index <= report.max_fault_index; ++fault_index)
    {
        auto fixture = make_fixture();
        const auto baseline = capture(fixture);
        auto invoke_once = [&](allocation_fault::SweepRunContext &context) {
            context.MarkMethodInvoked();
            return invoke(fixture, context);
        };
        ...
        allocation_fault::ResetInjectedFailureCount();
        {
            allocation_fault::FailAfter fault(fault_index);
            try
            {
                const auto invocation = allocation_fault::InvokeForSweep(invoke_once, context);
        ...
        const auto injected = allocation_fault::InjectedFailureCount();
        report.saw_injected_failure = report.saw_injected_failure || injected > 0;
        iteration.postcondition_met = static_cast<bool>(verify(iteration, baseline, fixture));
...
[[nodiscard]] inline bool PassedObservedMutationSweep(const allocation_fault::SweepReport &report) noexcept
{
    const auto passed = report.Passed() && report.saw_success &&
                        (report.saw_injected_failure || report.observed_allocations == 0) &&
                        report.observed_allocations >= 0 && report.max_fault_index >= report.observed_allocations;
    if (!passed)
    {
        std::cerr << report.api << " mutation sweep failed: observed=" << report.observed_allocations
                  ...
                  << " saw_injected_failure=" << report.saw_injected_failure
