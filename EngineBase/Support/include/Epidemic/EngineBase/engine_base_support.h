...
namespace detail
{
template <typename TValue> [[nodiscard]] inline TValue UnwrapOrThrow(epidemic::foundation::Result<TValue> result)
{
    if (!result.HasValue())
    {
...
RegisterEngineBase(core::Application &application, EngineBaseOptions options)
{
...
[[nodiscard]] inline std::shared_ptr<platform::IPlatformRuntime> RegisterWindowsRuntime(core::Application &application)
{
...
[[nodiscard]] inline std::shared_ptr<input::IInputSystem> RegisterInputRuntime(core::Application &application)
{
...
[[nodiscard]] inline GraphicsRuntimeServices RegisterGraphicsRuntime(core::Application &application,
                                                                    GraphicsRuntimeOptions options)
{
    ...
    case GraphicsBackend::Null:
        device = detail::UnwrapOrThrow(rhi::CreateNullRhiDevice(device_desc));
        break;
    case GraphicsBackend::D3D11:
        device = detail::UnwrapOrThrow(rhi::d3d11::CreateD3D11RhiDevice(device_desc));
        break;
    }

    auto command_context = detail::UnwrapOrThrow(device->CreateCommandContext());
    ...
    return GraphicsRuntimeServices{std::move(device), std::move(command_context)};
}

[[nodiscard]] inline std::shared_ptr<platform::IWindow>
CreateMainWindow(core::Application &application, const platform::WindowCreateInfo &create_info)
{
    ...
    return detail::UnwrapOrThrow(window_system->CreateWindow(create_info));
}

[[nodiscard]] inline std::shared_ptr<rhi::IRhiSwapChain>
                      ...
                      const rhi::RhiSwapChainDesc &desc)
{
    ...
    auto swap_chain = detail::UnwrapOrThrow(device->CreateSwapChain(swap_chain_desc));
...
inline void RegisterInputFrameLoop(core::Application &application)
{
    auto frame_platform_events = EnsureFramePlatformEvents(application);
